...
uint32_t readAccel() {
	...
	writeSensorRegisterMMA8451Q(0x0E, 0x01, i2cPullUpValue); //set 4g scale - can only do this in standby modeoe
	...
	for (int i=0; i<numberOfReadings; i++) {
		...
		if (i2cReadStatus == kWarpStatusOK) {
			...
			xdata[i] = (int) (readSensorRegisterValueCombined/208 + 0.5);
		}	
		else {
//			SEGGER_RTT_printf(0, "\nError reading register");
	...
	for (int i=0; i<numberOfReadings; i++) {
		...
		if (i < numberOfReadings-1) {
			velocity[i] = (xdata[i] + xdata[i+1]) * delay/20;
			if (i > 0) {
				velocity[i] += velocity[i-1];

				distance[i-1] = (velocity[i-1] + velocity[i]) * delay/2000;
				if (i > 1) {
	...
	SEGGER_RTT_printf(0, "Acceleration on impact is %dm/s^2\n", maxAccel);
